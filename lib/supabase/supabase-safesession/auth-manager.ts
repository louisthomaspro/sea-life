...
import { AuthResponse, AuthTokens, SupabaseJwtPayload } from "@/lib/supabase/supabase-safesession/types"

// https://github.com/Zanzofily/supabase-safesession/blob/main/src/auth.server.ts
...
class AuthManager {
   ...
   */
  private async parseAndVerifySession(tokens: AuthTokens): Promise<AuthResponse> {
    try {
      const session = jwt.verify(tokens.access_token, this.jwtSecret) as SupabaseJwtPayload
