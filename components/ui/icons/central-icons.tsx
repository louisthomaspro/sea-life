export function CentralDiamondFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M9.50059 8L7.50059 10L9.50059 12M13.4148 19.5858L21.5287 11.4719C22.3319 10.6687 22.3057 9.35857 21.471 8.58809L17.0752 4.53039C16.7057 4.18937 16.2214 4 15.7186 4H8.28256C7.77978 4 7.29545 4.18937 6.926 4.53039L2.53016 8.58809C1.69548 9.35857 1.66929 10.6687 2.47251 11.4719L10.5864 19.5858C11.3674 20.3668 12.6338 20.3668 13.4148 19.5858Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralRulerFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M8.9997 8.99994L11.2497 11.2499M8.9997 8.99994L12.4997 5.49994M8.9997 8.99994L5.4997 12.4999M12.4997 5.49994L14.5855 3.41416C15.3665 2.63311 16.6329 2.63311 17.4139 3.41416L20.5855 6.58573C21.3665 7.36678 21.3665 8.63311 20.5855 9.41416L9.41391 20.5857C8.63286 21.3668 7.36653 21.3668 6.58548 20.5857L3.41391 17.4142C2.63286 16.6331 2.63286 15.3668 3.41391 14.5857L5.4997 12.4999M12.4997 5.49994L13.7497 6.74994M5.4997 12.4999L6.7497 13.7499"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CustomDepthIcon(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M4 10C5.35118 10 6.68014 8.2 8 8.2C9.33812 8.2 10.6669 10 12 10C13.3246 10 14.6534 8.2 16 8.2C17.3124 8.2 18.6416 10 20 10M4 4.8C5.35118 4.8 6.68014 3 8 3C9.33812 3 10.6669 4.8 12 4.8C13.3246 4.8 14.6534 3 16 3C17.3124 3 18.6416 4.8 20 4.8"
        stroke="black"
        strokeWidth="2"
        strokeLinecap="round"
      />
      <path
        d="M15.5 19L12.5 22L9.5 19M12.5 21.4483V14.5"
        stroke="black"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralHomeOpenFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M20 18V9.90754C20 9.33081 19.751 8.78216 19.317 8.40238L13.317 3.15238C12.563 2.49259 11.437 2.49259 10.683 3.15238L4.68299 8.40238C4.24896 8.78216 4 9.33081 4 9.90754V18C4 19.1046 4.89543 20 6 20H9C9.55228 20 10 19.5523 10 19V16C10 14.8954 10.8954 14 12 14C13.1046 14 14 14.8954 14 16V19C14 19.5523 14.4477 20 15 20H18C19.1046 20 20 19.1046 20 18Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralHomeRoundDoorFilledOnStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M13.6996 3.45488C12.6758 2.75104 11.3242 2.75104 10.3004 3.45488L4.30041 7.57988C3.48635 8.13954 3 9.06411 3 10.052V17.9999C3 19.6568 4.34315 20.9999 6 20.9999H8.5C9.05228 20.9999 9.5 20.5522 9.5 19.9999V16.4999C9.5 15.1192 10.6193 13.9999 12 13.9999C13.3807 13.9999 14.5 15.1192 14.5 16.4999V19.9999C14.5 20.5522 14.9477 20.9999 15.5 20.9999H18C19.6569 20.9999 21 19.6568 21 17.9999V10.052C21 9.06411 20.5136 8.13954 19.6996 7.57988L13.6996 3.45488Z"
        fill="currentColor"
      />
    </svg>
  )
}

export function CentralEarthFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M4.17156 8.07011L6.85387 10.0833C7.50915 10.5752 8.44975 10.3548 8.81836 9.62306C8.99021 9.28194 9.30184 9.03235 9.67226 8.93916L11.8457 8.39241C12.5416 8.21734 13.0847 7.67348 13.2587 6.97732L14.126 3.50882M21 12C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12ZM11.4252 15.9984L10.6023 14.7641C10.2853 14.2886 10.3486 13.6553 10.7535 13.2519C10.9966 13.0096 11.3322 12.8834 11.6748 12.9052L12.5015 12.958C12.8453 12.9799 13.1771 13.0927 13.463 13.2848L14.8342 14.2063C15.4406 14.6138 15.6501 15.4083 15.3233 16.0618C15.0772 16.554 14.5742 16.8649 14.0239 16.8649H13.0443C12.3937 16.8649 11.7861 16.5397 11.4252 15.9984Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralBookmarkFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M19 19.9948V5C19 3.89543 18.1046 3 17 3H7C5.89543 3 5 3.89543 5 5V19.9948C5 20.8047 5.91243 21.2787 6.57503 20.813L10.8499 17.8083C11.54 17.3233 12.46 17.3233 13.1501 17.8083L17.425 20.813C18.0876 21.2787 19 20.8047 19 19.9948Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralGroup2FilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M15.2755 13.1471C18.0038 12.4838 21.0814 14.0633 22.1462 17.8855C22.4575 19.0029 21.5151 20 20.3552 20H17.0013M10.5013 7C10.5013 8.65685 9.15819 10 7.50133 10C5.84448 10 4.50133 8.65685 4.50133 7C4.50133 5.34315 5.84448 4 7.50133 4C9.15819 4 10.5013 5.34315 10.5013 7ZM19.5013 7C19.5013 8.65685 18.1582 10 16.5013 10C14.8445 10 13.5013 8.65685 13.5013 7C13.5013 5.34315 14.8445 4 16.5013 4C18.1582 4 19.5013 5.34315 19.5013 7ZM11.3554 20H3.64728C2.48733 20 1.54546 18.9995 1.85716 17.8822C3.67324 11.3726 11.3294 11.3726 13.1455 17.8823C13.4572 18.9995 12.5153 20 11.3554 20Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralPeopleFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M15.5011 6.5C15.5011 8.433 13.9341 10 12.0011 10C10.0681 10 8.50112 8.433 8.50112 6.5C8.50112 4.567 10.0681 3 12.0011 3C13.9341 3 15.5011 4.567 15.5011 6.5Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinejoin="round"
      />
      <path
        d="M12.0011 13C8.60997 13 6.03711 15.0143 4.9836 17.8629C4.5748 18.9682 5.51944 20 6.69796 20H17.3043C18.4828 20 19.4274 18.9682 19.0186 17.8629C17.9651 15.0143 15.3923 13 12.0011 13Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralPeopleFilledOnStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M12 2C9.51472 2 7.5 4.01472 7.5 6.5C7.5 8.98528 9.51472 11 12 11C14.4853 11 16.5 8.98528 16.5 6.5C16.5 4.01472 14.4853 2 12 2Z"
        fill="currentColor"
      />
      <path
        d="M12.0015 12C8.16958 12 5.23686 14.2962 4.04605 17.516C3.70562 18.4365 3.93869 19.3438 4.47985 19.9887C5.00722 20.6172 5.82133 21 6.69832 21H17.3046C18.1816 21 18.9957 20.6172 19.5231 19.9887C20.0643 19.3438 20.2973 18.4365 19.9569 17.516C18.7661 14.2962 15.8334 12 12.0015 12Z"
        fill="currentColor"
      />
    </svg>
  )
}

export function CentralHomeFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M4 9.7731C4 9.21357 4 8.9338 4.07063 8.67514C4.1332 8.44597 4.23609 8.22979 4.3745 8.03672C4.53071 7.81879 4.74784 7.64237 5.1821 7.28953L9.9821 3.38953C10.7014 2.8051 11.0611 2.51288 11.4609 2.40096C11.8135 2.30226 12.1865 2.30226 12.5391 2.40096C12.9389 2.51288 13.2986 2.8051 14.0179 3.38953L18.8179 7.28954C19.2522 7.64237 19.4693 7.81879 19.6255 8.03672C19.7639 8.22979 19.8668 8.44597 19.9294 8.67514C20 8.9338 20 9.21357 20 9.7731V16.8C20 17.9201 20 18.4802 19.782 18.908C19.5903 19.2843 19.2843 19.5903 18.908 19.782C18.4802 20 17.9201 20 16.8 20H7.2C6.07989 20 5.51984 20 5.09202 19.782C4.71569 19.5903 4.40973 19.2843 4.21799 18.908C4 18.4802 4 17.9201 4 16.8V9.7731Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralCircleCheckFilledOnStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2ZM15.774 10.1333C16.1237 9.70582 16.0607 9.0758 15.6332 8.72607C15.2058 8.37635 14.5758 8.43935 14.226 8.86679L10.4258 13.5116L9.20711 12.2929C8.81658 11.9024 8.18342 11.9024 7.79289 12.2929C7.40237 12.6834 7.40237 13.3166 7.79289 13.7071L9.79289 15.7071C9.99267 15.9069 10.2676 16.0129 10.5498 15.9988C10.832 15.9847 11.095 15.8519 11.274 15.6333L15.774 10.1333Z"
        fill="currentColor"
      />
    </svg>
  )
}

export function CentralBookmarkCheckFilledOnStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M8.16146 2H15.8385C16.3657 1.99998 16.8205 1.99997 17.195 2.03057C17.5904 2.06287 17.9836 2.13419 18.362 2.32698C18.9265 2.6146 19.3854 3.07354 19.673 3.63803C19.8658 4.01641 19.9371 4.40963 19.9694 4.80497C20 5.17954 20 5.63432 20 6.16146L20 18.9346C20 19.3993 20.0001 19.8149 19.9698 20.1459C19.9398 20.4742 19.8687 20.909 19.5833 21.2788C19.2205 21.7489 18.6675 22.0334 18.0741 22.0554C17.6073 22.0728 17.2122 21.8779 16.9276 21.7115C16.6407 21.5438 16.3025 21.3021 15.9244 21.0321L12.3487 18.478C12.1705 18.3507 12.0771 18.2846 12.0065 18.2412L12 18.2372L11.9935 18.2412C11.9229 18.2846 11.8295 18.3507 11.6513 18.478L8.07564 21.032C7.69748 21.3022 7.35934 21.5437 7.07245 21.7115C6.7878 21.8779 6.39272 22.0728 5.92589 22.0554C5.33249 22.0334 4.77955 21.7489 4.41673 21.2788C4.13131 20.909 4.06022 20.4742 4.0302 20.1459C3.99995 19.8149 3.99997 19.3994 4 18.9347L4 6.16145C3.99999 5.63431 3.99997 5.17955 4.03057 4.80497C4.06288 4.40963 4.13419 4.01641 4.32699 3.63803C4.61461 3.07354 5.07355 2.6146 5.63803 2.32698C6.01641 2.13419 6.40963 2.06287 6.80498 2.03057C7.17954 1.99997 7.63433 1.99998 8.16146 2ZM15.2071 8.95711C15.5976 8.56658 15.5976 7.93342 15.2071 7.54289C14.8166 7.15237 14.1834 7.15237 13.7929 7.54289L11 10.3358L10.2071 9.54289C9.81658 9.15237 9.18342 9.15237 8.79289 9.54289C8.40237 9.93342 8.40237 10.5666 8.79289 10.9571L10.2929 12.4571C10.4804 12.6446 10.7348 12.75 11 12.75C11.2652 12.75 11.5196 12.6446 11.7071 12.4571L15.2071 8.95711Z"
        fill="currentColor"
      />
    </svg>
  )
}

export function CentralSquareGridCircleFilledOnStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M3.0025 5.99945C3.0025 4.34398 4.34453 3.00195 6 3.00195H8C9.65548 3.00195 10.9975 4.34398 10.9975 5.99945V7.99945C10.9975 9.65493 9.65548 10.997 8 10.997H6C4.34453 10.997 3.0025 9.65493 3.0025 7.99945V5.99945Z"
        fill="currentColor"
      />
      <path
        d="M3.0025 15.9995C3.0025 14.344 4.34453 13.002 6 13.002H8C9.65548 13.002 10.9975 14.344 10.9975 15.9995V17.9995C10.9975 19.6549 9.65548 20.997 8 20.997H6C4.34453 20.997 3.0025 19.6549 3.0025 17.9995V15.9995Z"
        fill="currentColor"
      />
      <path
        d="M13.0025 5.99945C13.0025 4.34398 14.3445 3.00195 16 3.00195H18C19.6555 3.00195 20.9975 4.34398 20.9975 5.99945V7.99945C20.9975 9.65493 19.6555 10.997 18 10.997H16C14.3445 10.997 13.0025 9.65493 13.0025 7.99945V5.99945Z"
        fill="currentColor"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M17 13.002C14.7922 13.002 13.0025 14.7917 13.0025 16.9995C13.0025 19.2072 14.7922 20.997 17 20.997C19.2078 20.997 20.9975 19.2072 20.9975 16.9995C20.9975 14.7917 19.2078 13.002 17 13.002ZM14.9975 16.9995C14.9975 15.8935 15.8941 14.997 17 14.997C18.106 14.997 19.0025 15.8935 19.0025 16.9995C19.0025 18.1054 18.106 19.002 17 19.002C15.8941 19.002 14.9975 18.1054 14.9975 16.9995Z"
        fill="currentColor"
      />
    </svg>
  )
}

export function CentralPlusLargeFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path d="M12 4V12M12 12V20M12 12H4M12 12H20" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
    </svg>
  )
}

export function CentralTrashCanFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M5.87554 19.133L6.87332 19.0665L5.87554 19.133ZM18.1245 19.133L17.1267 19.0665L18.1245 19.133ZM3 5C2.44772 5 2 5.44772 2 6C2 6.55228 2.44772 7 3 7V5ZM21 7C21.5523 7 22 6.55228 22 6C22 5.44772 21.5523 5 21 5V7ZM11 11C11 10.4477 10.5523 10 10 10C9.44772 10 9 10.4477 9 11H11ZM9 16C9 16.5523 9.44772 17 10 17C10.5523 17 11 16.5523 11 16H9ZM15 11C15 10.4477 14.5523 10 14 10C13.4477 10 13 10.4477 13 11H15ZM13 16C13 16.5523 13.4477 17 14 17C14.5523 17 15 16.5523 15 16H13ZM14.9056 6.24926C15.0432 6.78411 15.5884 7.1061 16.1233 6.96844C16.6581 6.83078 16.9801 6.28559 16.8424 5.75074L14.9056 6.24926ZM4.00221 6.06652L4.87775 19.1996L6.87332 19.0665L5.99779 5.93348L4.00221 6.06652ZM7.87111 22H16.1289V20H7.87111V22ZM19.1222 19.1996L19.9978 6.06652L18.0022 5.93348L17.1267 19.0665L19.1222 19.1996ZM19 5H5V7H19V5ZM3 7H5V5H3V7ZM19 7H21V5H19V7ZM16.1289 22C17.7083 22 19.0172 20.7754 19.1222 19.1996L17.1267 19.0665C17.0917 19.5918 16.6554 20 16.1289 20V22ZM4.87775 19.1996C4.98281 20.7754 6.29171 22 7.87111 22V20C7.34464 20 6.90834 19.5918 6.87332 19.0665L4.87775 19.1996ZM9 11V16H11V11H9ZM13 11V16H15V11H13ZM12 4C13.3965 4 14.5725 4.95512 14.9056 6.24926L16.8424 5.75074C16.2874 3.59442 14.3312 2 12 2V4ZM9.09447 6.24926C9.42756 4.95512 10.6035 4 12 4V2C9.66885 2 7.7126 3.59442 7.1576 5.75074L9.09447 6.24926Z"
        fill="currentColor"
      />
    </svg>
  )
}

export function CentralChevronLeftFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M15.0002 20L8.4144 13.4142C7.63335 12.6332 7.63335 11.3669 8.4144 10.5858L15.0002 4"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralLoaderFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M12.0003 3V6M12.0003 18V21M5.63634 5.63604L7.75766 7.75736M16.2429 16.2426L18.3643 18.364M3 12.0007H6M18 12.0007H21M5.63634 18.364L7.75766 16.2426M16.2429 7.75736L18.3643 5.63604"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralPencilLineFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M14 20.9999H21M20.5858 5.08573L18.9142 3.41416C18.1332 2.63311 16.8668 2.63311 16.0858 3.41416L3.58579 15.9142C3.21071 16.2892 3 16.7979 3 17.3284V20.9999H6.67157C7.20201 20.9999 7.71071 20.7892 8.08579 20.4142L20.5858 7.91416C21.3668 7.13311 21.3668 5.86678 20.5858 5.08573Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralMagnifyingGlassFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M20 20L16.05 16.05M18 11C18 14.866 14.866 18 11 18C7.13401 18 4 14.866 4 11C4 7.13401 7.13401 4 11 4C14.866 4 18 7.13401 18 11Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
      />
    </svg>
  )
}

export function CentralMagnifyingGlassFilledOnStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M11 15C13.2091 15 15 13.2091 15 11C15 8.79086 13.2091 7 11 7C8.79086 7 7 8.79086 7 11C7 13.2091 8.79086 15 11 15Z"
        fill="currentColor"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M11 5C7.68629 5 5 7.68629 5 11C5 14.3137 7.68629 17 11 17C14.3137 17 17 14.3137 17 11C17 7.68629 14.3137 5 11 5ZM3 11C3 6.58172 6.58172 3 11 3C15.4183 3 19 6.58172 19 11C19 12.8487 18.3729 14.551 17.3199 15.9056L20.7071 19.2929C21.0976 19.6834 21.0976 20.3166 20.7071 20.7071C20.3166 21.0976 19.6834 21.0976 19.2929 20.7071L15.9056 17.3199C14.551 18.3729 12.8487 19 11 19C6.58172 19 3 15.4183 3 11Z"
        fill="currentColor"
      />
    </svg>
  )
}

export function CentralCrossLargeFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path d="M5 5L19 19M19 5L5 19" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
    </svg>
  )
}

export function CentralTriangleExclamationFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M12 9.01917V12.0134M12 15H12.01M10.2765 3.99036L3.27481 15.998C2.49885 17.3288 3.45836 19 4.99836 19H19.0016C20.5416 19 21.5011 17.3288 20.7252 15.998L13.7235 3.99035C12.9536 2.66988 11.0464 2.66988 10.2765 3.99036ZM12.25 15C12.25 15.1381 12.1381 15.25 12 15.25C11.8619 15.25 11.75 15.1381 11.75 15C11.75 14.8619 11.8619 14.75 12 14.75C12.1381 14.75 12.25 14.8619 12.25 15Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
      />
    </svg>
  )
}

export function CentralShareFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M14.3354 7.37976C14.8349 8.34235 15.8406 9 17 9C18.6569 9 20 7.65685 20 6C20 4.34315 18.6569 3 17 3C15.3431 3 14 4.34315 14 6C14 6.49749 14.1211 6.9667 14.3354 7.37976ZM14.3354 7.37976L8.66458 10.6202M8.66458 10.6202C8.16512 9.65765 7.15936 9 6 9C4.34315 9 3 10.3431 3 12C3 13.6569 4.34315 15 6 15C7.15936 15 8.16512 14.3424 8.66458 13.3798M8.66458 10.6202C8.8789 11.0333 9 11.5025 9 12C9 12.4975 8.8789 12.9667 8.66458 13.3798M8.66458 13.3798L14.3354 16.6202M14.3354 16.6202C14.1211 17.0333 14 17.5025 14 18C14 19.6569 15.3431 21 17 21C18.6569 21 20 19.6569 20 18C20 16.3431 18.6569 15 17 15C15.8406 15 14.8349 15.6576 14.3354 16.6202Z"
        stroke="currentColor"
        strokeWidth="2"
      />
    </svg>
  )
}

export function CentralAtFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M16.7368 19.6541C15.361 20.5073 13.738 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12C21 13.9262 20.0428 15.9154 17.8101 15.7125C15.9733 15.5455 14.6512 13.8737 14.9121 12.0479L15.4274 8.5M14.8581 12.4675C14.559 14.596 12.8066 16.1093 10.9442 15.8476C9.08175 15.5858 7.81444 13.6481 8.11358 11.5196C8.41272 9.39109 10.165 7.87778 12.0275 8.13953C13.8899 8.40128 15.1573 10.339 14.8581 12.4675Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
      />
    </svg>
  )
}

export function CentralBarsThree2FilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M3 12H21M3 6H21M3 18H11"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralArrowBoxLeftFilledOffStroke2Radius2(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M11.25 20H6C4.89543 20 4 19.1046 4 18L4 6C4 4.89543 4.89543 4 6 4L11.25 4M20 12L8.75 12M20 12L15.5 16.5M20 12L15.5 7.5"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralSettingsGear2FilledOffStroke2Radius3(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M7.67668 5.32539L7.30515 5.23965C6.72652 5.10612 6.11991 5.28009 5.7 5.7C5.28009 6.11991 5.10612 6.72651 5.23965 7.30515L5.32539 7.67668C5.55082 8.65357 5.148 9.668 4.31382 10.2241L3.74885 10.6008C3.28101 10.9127 3 11.4377 3 12C3 12.5623 3.28101 13.0873 3.74885 13.3992L4.31382 13.7759C5.148 14.332 5.55082 15.3464 5.32539 16.3233L5.23965 16.6949C5.10612 17.2735 5.28009 17.8801 5.7 18.3C6.11991 18.7199 6.72651 18.8939 7.30515 18.7604L7.67668 18.6746C8.65357 18.4492 9.668 18.852 10.2241 19.6862L10.6008 20.2512C10.9127 20.719 11.4377 21 12 21C12.5623 21 13.0873 20.719 13.3992 20.2512L13.7759 19.6862C14.332 18.852 15.3464 18.4492 16.3233 18.6746L16.6949 18.7604C17.2735 18.8939 17.8801 18.7199 18.3 18.3C18.7199 17.8801 18.8939 17.2735 18.7604 16.6949L18.6746 16.3233C18.4492 15.3464 18.852 14.332 19.6862 13.7759L20.2512 13.3992C20.719 13.0873 21 12.5623 21 12C21 11.4377 20.719 10.9127 20.2512 10.6008L19.6862 10.2241C18.852 9.668 18.4492 8.65357 18.6746 7.67668L18.7604 7.30515C18.8939 6.72652 18.7199 6.11991 18.3 5.7C17.8801 5.28009 17.2735 5.10612 16.6949 5.23965L16.3233 5.32539C15.3464 5.55082 14.332 5.148 13.7759 4.31382L13.3992 3.74884C13.0873 3.28101 12.5623 3 12 3C11.4377 3 10.9127 3.28101 10.6008 3.74885L10.2241 4.31382C9.668 5.148 8.65357 5.55082 7.67668 5.32539Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinejoin="round"
      />
      <path
        d="M15 12C15 13.6569 13.6569 15 12 15C10.3431 15 9 13.6569 9 12C9 10.3431 10.3431 9 12 9C13.6569 9 15 10.3431 15 12Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinejoin="round"
      />
    </svg>
  )
}

export function CentralCheckmark2FilledOffStroke2Radius3(props: JSX.IntrinsicElements["svg"]) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M3 15L9.29412 20L21 4"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}
